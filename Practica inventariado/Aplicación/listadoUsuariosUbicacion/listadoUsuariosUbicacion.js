"use strict"
//# sourceURL=listadoUsuariosUbicacion.js

document.querySelector("#btnAceptarListadoUsuariosUbicacion").addEventListener("click", validarListadoUsuariosUbicacion, false);

function validarListadoUsuariosUbicacion(){
    let sErrores = "";
    let bValido = true; // en principio el formulario es válido

    let id = frmListadoUsuariosUbicacion.txtIdUbicacion.value.trim();
    let oExpReg = /^[0-9\s]{1,9}$/;

    if (oExpReg.test(id) == false) {
        if (bValido == true) { // ==> Primer error detectado en este campo
            frmListadoUsuariosUbicacion.txtIdUbicacion.focus();
            bValido = false;
        }
        frmListadoUsuariosUbicacion.txtIdUbicacion.classList.add("error");
        sErrores += "El id solo puede tener numeros de como maximo 9 cifras.\n";
    }
    else{
        frmListadoUsuariosUbicacion.txtIdUbicacion.classList.remove("error");
    }

    if (bValido){
        mostrarListadoUsuariosUbicacion();
    }
    else{
        alert(sErrores);
    }
...
function mostrarListadoUsuariosUbicacion(oEvento){
    var id = {
        idUbicacion: frmListadoUsuariosUbicacion.txtIdUbicacion.value.trim(),
    }
    var sParametros = "datosListadoUbicacion=" + JSON.stringify(id);
    $.get("listadoUsuariosUbicacion/listadoUsuariosUbicacion.php", sParametros, procesoRespuestaMostrarListadoUsuariosUbicacion, "html");
}
function procesoRespuestaMostrarListadoUsuariosUbicacion(oDatos, sStatus, oXHR){
